...
def generate_douban_book(model_type, model_name, book_type, care_thing):
    config.llm_model_type = model_type
    ...
    markdown_file_path = douban_new_book.run()
    report, report_file_path = report_generator.generate_douban_book_report(markdown_file_path, care_thing)
...
with gr.Blocks(title="GitHubSentinel") as demo:
    ...
    # 创建 豆瓣 新书和热门图书 Tab
    with gr.Tab("豆瓣 新书和热门图书"):
        gr.Markdown("## 豆瓣 新书和热门图书")  # 添加小标题

        # 创建 Radio 组件
        model_type = gr.Radio(["openai"], label="模型类型", info="使用 OpenAI GPT API 或 Ollama 私有化模型服务")

        # 创建 Dropdown 组件
        model_name = gr.Dropdown(choices=["gpt-4o", "gpt-4o-mini", "gpt-3.5-turbo"], label="选择模型")

        # 创建关于图书类型的 多选 组件
        book_type = gr.CheckboxGroup(label="图书类型", choices=["全部", "文学", "小说", "历史文化", "社会纪实", "科学新知", "艺术设计", "影视戏剧"])

        # 创建输入框组件
        care_thing = gr.Textbox(label="关注的事情", placeholder="请输入您关注的事情", lines=3)

        # 使用 radio 组件的值来更新 dropdown 组件的选项
        # model_type.change(fn=update_model_list, inputs=model_type, outputs=model_name)

        # 创建按钮来生成报告
        button = gr.Button("生成豆瓣当前新书和热门图书内容")
        ...
        # 将按钮点击事件与导出函数绑定
        button.click(generate_douban_book, inputs=[model_type, model_name, book_type, care_thing,], outputs=[markdown_output, file_output])
