...
def hackernews_job(hacknews_client, notifier, report_generator):
    LOG.info("[开始执行HackerNews定时任务]")
    markdown_file_path = hacknews_client.export_hackernews_top_stories()
    # 从Markdown文件自动生成进展简报
...
def main():
    ...
    hacknews_client = HackerClient()  # 创建HackerNews客户端实例
    notifier = Notifier(config.email)  # 创建通知器实例
    llm_github = LLM()  # 创建语言模型实例
    report_generator_github = ReportGenerator(llm_github)  # 创建报告生成器实例
    subscription_manager = SubscriptionManager(config.subscriptions_file)  # 创建订阅管理器实例

    # 启动时立即执行（如不需要可注释）
    github_job(subscription_manager, github_client, report_generator_github, notifier, config.freq_days)

    llm_hackernews = LLM('hackernews')  # 创建语言模型实例
    report_generator_hackernews = ReportGenerator(llm_hackernews)  # 创建报告生成器实例
    hackernews_job(hacknews_client, notifier, report_generator_hackernews)

    # 启动hacknews定时任务
    schedule.every(config.freq_days).days.at(
        config.exec_time
    ).do(hackernews_job, hacknews_client, notifier, report_generator_hackernews)

    # 安排每天的定时任务
    schedule.every(config.freq_days).days.at(
        config.exec_time
    ).do(github_job, subscription_manager, github_client, report_generator_github, notifier, config.freq_days)
